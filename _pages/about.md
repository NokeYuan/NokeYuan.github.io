---
layout: about
title: about
permalink: /
subtitle:

profile:
  align: right
  image: prof_pic.jpg
  image_circular: false # crops the image to make it circular
  more_info:

selected_papers: true # includes a list of papers marked as "selected={true}"
social: false # includes social icons at the bottom of the page

announcements:
  enabled: false
  scrollable: true
  limit: 5

latest_posts:
  enabled: false
  scrollable: true
  limit: 3
---

<div class="contact-links" style="margin-bottom: 1rem;">
  <a href="https://scholar.google.ca/citations?user=BCUhIHYAAAAJ&hl=en" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">[Google Scholar]</a>
  <a href="https://github.com/NokeYuan" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">[GitHub]</a>
  <a href="https://www.linkedin.com/in/nokeyuan" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">[LinkedIn]</a>
</div>

I'm a PhD candidate at the University of Toronto working on AI in healthcare, advised by [Prof. Geoffrey Liu](https://www.uhncombiel.com/about-the-liu-lab) and [Dr. Robert Grant](https://scholar.google.com/citations?user=JZTeUIYAAAAJ&hl=en). My research lies at the intersection of artificial intelligence and healthcare — spanning clinical machine learning, computational genomics, large language models, and computer vision — and is based at the [Princess Margaret Cancer Centre](https://www.uhn.ca/PrincessMargaret), where I build machine learning systems for real-time cancer symptom prediction and clinical decision support.

On the industry side, I'm currently a [Machine Learning Associate](https://vectorinstitute.ai/research-talent/students/machine-learning-associates/) at the [Vector Institute](https://vectorinstitute.ai/), building a full-stack computer vision system with [Deighton Associates](https://deighton.com/) for automated roadway asset assessment from dash-cam imagery. Previously, at the [UHN AI Hub](https://uhnhub.ai/), we designed and developed [Clinical-Camel-ASR](https://github.com/alif-munim/clinical-camel-asr) — an end-to-end medical scribing framework, extending [Clinical Camel](https://arxiv.org/abs/2305.12031), that turns doctor–patient conversations into structured clinical summaries — in collaboration with [WangLab](https://scholar.google.com/citations?user=37FDILIAAAAJ&hl=en) (Dr. Bo Wang), supervised by [Dr. Jun Ma](https://scholar.google.com/citations?user=bW1UV4IAAAAJ&hl=en) and [Alif Munim](https://scholar.google.com/citations?user=3XRvwJwAAAAJ&hl=en).

A selection of my publications is below — see the [publications](/publications/) page for the full list.

<h2 class="experience-heading">Experience</h2>

<div class="experience-list">
  <div class="experience">
    <div class="dates">Apr 2026 – Present<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">Toronto, Canada</span></div>
    <div class="details">
      <div class="place">Vector Institute</div>
      <div class="role">Machine Learning Associate</div>
      <div class="description">Full-stack computer vision system (FastAPI + React) with Deighton Associates, automating roadway asset inventory and pavement-condition assessment from dash-cam imagery.</div>
    </div>
    <img class="logo" src="/assets/img/vector.png" alt="Vector Institute logo" loading="lazy">
  </div>
  <div class="experience">
    <div class="dates">May 2025 – Oct 2025<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">Toronto, Canada</span></div>
    <div class="details">
      <div class="place">UHN AI Hub</div>
      <div class="role">Machine Learning Engineer Intern</div>
      <div class="description">Designed and developed Clinical-Camel-ASR, an end-to-end medical scribing framework extending Clinical Camel, converting doctor–patient conversations into structured clinical summaries using ASR and LLMs, with WangLab.</div>
    </div>
    <img class="logo" src="/assets/img/uhn.svg" alt="University Health Network logo" loading="lazy">
  </div>
  <div class="experience">
    <div class="dates">Jan 2022 – May 2022<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">London, UK</span></div>
    <div class="details">
      <div class="place">UCL Institute of Health Informatics</div>
      <div class="role">Machine Learning Intern</div>
      <div class="description">Multi-task deep learning for detection of age-related macular degeneration from retinal fundus images; CNN-based COVID-19 detection from lung MRI.</div>
    </div>
    <img class="logo" src="/assets/img/ucl.svg" alt="UCL logo" loading="lazy">
  </div>
  <div class="experience">
    <div class="dates">Apr 2020 – Sep 2020<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">Beijing, China</span></div>
    <div class="details">
      <div class="place">NetEase</div>
      <div class="role">Data Analyst Intern</div>
      <div class="description">Statistical risk models over 1M+ user records for supplier risk management and strategic decision-making.</div>
    </div>
    <img class="logo" src="/assets/img/netease.png" alt="NetEase logo" loading="lazy">
  </div>
</div>

<h2 class="experience-heading">Education</h2>

<div class="experience-list">
  <div class="experience">
    <div class="dates">Sep 2022 – Present<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">Toronto, Canada</span></div>
    <div class="details">
      <div class="place">University of Toronto</div>
      <div class="role">PhD, AI in Healthcare — <a href="https://ims.utoronto.ca/">Institute of Medical Science</a></div>
      <div class="description">GPA: 4.0/4.0. Advisors: Prof. Geoffrey Liu, Dr. Robert Grant. Research: ML systems for real-time cancer symptom prediction and clinical decision support.</div>
    </div>
    <img class="logo" src="/assets/img/uoft.png" alt="University of Toronto logo" style="height: 3.6rem;" loading="lazy">
  </div>
  <div class="experience">
    <div class="dates">Sep 2020 – Sep 2021<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">London, UK</span></div>
    <div class="details">
      <div class="place">University College London</div>
      <div class="role">MSc, Health Data Science — <a href="https://www.ucl.ac.uk/population-health-sciences/health-informatics">Institute of Health Informatics</a></div>
      <div class="description">Advisor: <a href="https://profiles.ucl.ac.uk/48846-tom-lumbers">Prof. Tom Lumbers</a>. Focus: computational genomics.</div>
    </div>
    <img class="logo" src="/assets/img/ucl.svg" alt="UCL logo" loading="lazy">
  </div>
  <div class="experience">
    <div class="dates">Sep 2015 – Nov 2019<br><span class="location" style="font-size:0.8rem; color: var(--global-text-color-light);">Toronto, Canada</span></div>
    <div class="details">
      <div class="place">University of Toronto</div>
      <div class="role">HBSc, Double Specialist in Computer Science and Statistics — <a href="https://web.cs.toronto.edu/">Department of Computer Science</a> (St. George)</div>
      <div class="description"></div>
    </div>
    <img class="logo" src="/assets/img/uoft.png" alt="University of Toronto logo" style="height: 3.6rem;" loading="lazy">
  </div>
</div>
